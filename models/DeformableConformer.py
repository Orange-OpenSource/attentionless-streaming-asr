...
from tvdcn.ops import DeformConv1d
from tvdcn.ops import PackedDeformConv1d
...
class LinearProjection(nn.Module):
    ...
    def forward(self,
        ...
        if self.after_conv is not None:
            out = self.after_conv(out)

        return out

class DeformConv1dWrapper(PackedDeformConv1d):
    """
    Deformable convolutional layer for 1D inputs.
    This is a wrapper around PackedDeformConv1d to ensure compatibility with
    ...
    def reset_parameters(self):
        if not hasattr(self, 'modulated'):
            return
        DeformConv1d.reset_parameters(self)
    ...
    def offset_activation(self, offset, input_size):
        """    Apply activation function to the offset tensor.
        the offsets are in the range of [-input_size, input_size]
        Args:
            offset (torch.Tensor): The offset tensor.
            input_size (int): The size of the input tensor.
        Returns:
            torch.Tensor: The activated offset tensor.
        """
        return torch.tanh(offset)*input_size

    def forward(self, input):
        if self.deformable and self.offset_generator is not None:
            offset = self.offset_generator(input)
            offset = self.offset_activation(offset, input.shape[-1])
        else:
            offset = None
        mask = torch.ones_like(offset).detach() if offset is not None else None

        return self._conv_forward(input, self.weight, offset, mask, self.bias)

class MultiHeadDeformConv1D(nn.Module):
    def __init__(self,
                 in_channels,
                 out_channels,
                 kernel_sizes,
                 dropout=0.1,
                 after_conv=False,
                 deformable_config=True,
                 ):
        super().__init__()
        self.n_heads = len(kernel_sizes)
        if deformable_config:
            # Use the new DeformConv1dWrapper for better compatibility
            self.conv = DeformConv1dWrapper(
                in_channels,
                out_channels,
                ...
                groups=self.n_heads,
                offset_groups=self.n_heads,
            )
        else: # applying standard convolution
            self.conv = nn.Conv1d(
                in_channels,
                out_channels,
                kernel_size=kernel_sizes[0],
    ...
    def forward(self,
        ...
        if dynchunktrain_config is not None:
            ...
            out = F.pad(x, (self.padding, final_right_padding), value=0)
            ...
            #traspose the input before applying the convolution
            x = x.transpose(1, 2)  # [batch_size, in_channels, time_steps]
            out = self.conv(x)
            out = out.transpose(1, 2)  # [batch_size, time_steps, out_channels]
            if self.after_conv is not None:
                out = self.after_conv(out)
...
class StaticDeformConv1D(nn.Module):
    def __init__(self,
                 in_channels,
                 out_channels,
                 kernel_size,
        ...
        try:
            local_device = torch.distributed.get_rank()
        except Exception as e:
            print(f"Error getting local device rank: {e}")
            local_device = 0
        self.offset = nn.Parameter(torch.zeros(1, kernel_size * groups, 1).float().to(f"cuda:{local_device}"))  # Dummy offset
        self.padding = 0 #(kernel_size - 1) * 2 ** (dilation - 1) // 2
        if after_conv:
            self.after_conv = nn.Sequential(
                nn.LayerNorm(out_channels),
                Swish(),
    ...
    def forward(self,
        ...
        if dynchunktrain_config is not None:
            ...
            # -> [batch_size * num_chunks, in_channels, lc+chunk_size+rpad]
            activated_offset = self.offset_activation(
                self.offset, out.shape[-1]).repeat(out.shape[0],
                                                                  1, out.shape[-1])
            mask = torch.ones_like(activated_offset).detach()
            out = self.conv(out, activated_offset, mask=mask)
            out = out.transpose(1, 2)
            ...
            #traspose the input before applying the convolution
            x = x.transpose(1, 2)  # [batch_size, in_channels, time_steps]
            activated_offset = self.offset.repeat(x.shape[0], 1, x.shape[-1])
            mask = torch.ones_like(activated_offset).detach()
            out = self.conv(x, activated_offset, mask=mask)
            out = out.transpose(1, 2)  # [batch_size, time_steps, out_channels]
            if self.after_conv is not None:
                out = self.after_conv(out)
...
class ConformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if attention_type == "regularMHA":
            ...
            self.mhdcn_kernel_size = mhdcn_kernel_size
            if mhdcn_kernel_size == 0:
                #we will use a linear projection instead of deformable convolution
                self.mha_layer = LinearProjection(
                    in_channels=d_model,
                    out_channels=d_model,
                    after_conv=after_conv,
                )
            elif mhdcn_kernel_size < 0:
                self.mha_layer = None
            else:
                self.mha_layer = MultiHeadDeformConv1D(
                    in_channels=d_model,
                    out_channels=d_model,
                    kernel_sizes=[self.mhdcn_kernel_size] * nhead,
                    after_conv=after_conv,  # type: ignore
                    deformable_config=deformable_config,  # type: ignore
                )
            #endregion deformable convolution
...
class ConformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        layerdrop_prob=0.0,
        after_conv=False,
        mhdcn_kernel_size=5,  # for deformable convolution
        deformable_config=True,
    ):
        super().__init__()
        ...
        self.layers = torch.nn.ModuleList(
            [
                ConformerEncoderLayer(
                    ...
                    after_conv=after_conv,
                    mhdcn_kernel_size=mhdcn_kernel_size,
