...
class TransformerASR(TransformerInterface):
    ...
    def encode(
        ...
        if src.dim() == 4:
            ...
            #     x = x.permute(0,2,3,1)#transpose(1,2).transpose(2,3)
            #     # -> [batch_size, in_channels, lc+t+final_right_padding]
            #     out = torch.nn.functional.pad(x, (0, final_right_padding), value=0)
    
            #     # -> [batch_size, in_channels, num_chunks, lc+chunk_size]
            #     out = out.unfold(3,size=chunk_size, step=chunk_size)
            #     # -> [batch_size, num_chunks, in_channels, lc+chunk_size+rpad]
            #     out = out.permute(0,3,4,1,2)#transpose(3, 2).transpose(2,1)
            #     src = out.flatten(start_dim=0, end_dim=1)
        ...
        outputs = self.encoder(
            ...
            dynchunktrain_config=dynchunktrain_config,#None
        )
        ...
        if self.output_hidden_states:
            ...
            encoder_out, _ = outputs
            # if torch.cuda.is_available():
            #     encoder_out = torch.unflatten(encoder_out, dim=0, sizes=(real_batch_size, -1))
            #     encoder_out = torch.flatten(encoder_out, start_dim=1, end_dim=2)
            return encoder_out
