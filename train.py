...
from utils.loss.transducer_loss import transducer_loss
import os
...
class ASR(sb.Brain):
    def __init__(
        ...
        self.distill_loss_train=None
        self.ctc_loss_train = None
        self.transducer_loss_train = None
        self.n_batches = 0
    ...
    def compute_forward(self, batch, stage):
        ...
        if hasattr(self.hparams, "stateless_dec") and self.hparams.stateless_dec:
            h = self.modules.dec(e_in)
        else:
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            if hasattr(self.hparams, "fast_emit") and self.hparams.fast_emit:
                blank = self.hparams.blank_index
                loss_transducer = transducer_loss(
                    logits_transducer, tokens, wav_lens, token_lens,blank,c_fast_emit=self.hparams.c_fast_emit
                )
            ...
            # Adding each auxilliary loss in tensorboard stats
            with torch.no_grad():
                if self.transducer_loss_train is None:
                    self.ctc_loss_train = CTC_loss
                    self.transducer_loss_train = loss_transducer
                else:
                    self.ctc_loss_train += CTC_loss
                    self.transducer_loss_train += loss_transducer
            self.n_batches += 1
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.TRAIN:
            # Adding distillation loss into Tensorboard Stats
            if self.distill_loss_train is not None and self.n_batches != 0:
                stage_stats["Distillation Loss"] = self.distill_loss_train/self.n_batches
                #reinitialize the stats for the next stage/epoch
                self.distill_loss_train = None
            
            if self.ctc_loss_train is not None and self.n_batches != 0:
                stage_stats["CTC Loss"] = self.ctc_loss_train/self.n_batches
                #reinitialize the stats for the next stage/epoch
                self.ctc_loss_train = None
            if self.transducer_loss_train is not None and self.n_batches != 0:
                stage_stats["Transducer Loss"] = self.transducer_loss_train/self.n_batches
                #reinitialize the stats for the next stage/epoch
                self.transducer_loss_train = None
            #reinitialize the stats for the next stage/epoch
            self.n_batches = 0
...
def dataio_prepare(hparams):
    ...
    @sb.utils.data_pipeline.provides("sig","wav")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Error initializing DDP group: {e}")
    ...
    # If we are parsing ted_lium dataset
    if "dataset" in hparams.keys() and hparams["dataset"]=="tedlium2":
        if not os.path.exists(hparams["clipped_utt_folder"]):
            os.makedirs(hparams["clipped_utt_folder"])
        
        from tedlium2_prepare import prepare_tedlium2
    ...
    # keep a backup of checkpoints directory before averaging
    # them in test phase (skip if it exists)
    try:
        import shutil
        shutil.copytree(hparams["output_wer_folder"]+"save/",hparams["output_wer_folder"]+"backup/",ignore_dangling_symlinks=True)
